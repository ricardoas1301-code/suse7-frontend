...
assert("M6-UX-PREOAUTH alert triangle icon", preConfirmModal.includes('name="AlertTriangle"'));
assert("M6-UX-PREOAUTH important styling", preConfirmCss.includes("notice--important"));
...
assert("M6-UX-PREOAUTH panel operation illustration", readFileSync(join(feRoot, "../src/features/dashboard/operationalTasks/S7OperationalTasksPanel.jsx"), "utf8").includes("onboarding-operacao-comercial.png"));
...
assert(
  "COPY-PROGRESS-POLISH intro copy once",
  (sectionJsx.match(/Completar estas etapas é essencial para preparar sua operação e começar a usar o SUSE7\./g) ?? []).length === 1,
);
assert("COPY-PROGRESS-POLISH intro helper class", sectionJsx.includes("s7-configuration-onboarding__intro"));
{
  const headerBlock =

...
import { useDestructiveConfirmationCode } from "../components/ui/useDestructiveConfirmationCode";
...
} from "../features/concorrencia/domain/competitionHealthListClassifiers.js";
import {
...
} from "../features/concorrencia/domain/competitionContextualFilterPresentation.js";
import {
...
function CompetitorCompactCell({
  competitor,
  indiceColuna,
  precoNosso,
  onOpenDetail,
  ...
  return (
    <button
      ...
      className={[
        ...
        contextualMuted ? "concorrencia-catalog__comp--contextual-muted" : "",
      ]
        .filter(Boolean)
        .join(" ")}
      onClick={(e) => {
    ...
    >
      ...
      <div className="products-catalog__cell products-catalog__cell--product">
        <S7CatalogListingHeadline
          className="concorrencia-catalog__listing-headline concorrencia-catalog__headline-vendas-parity"
          title={name}
      ...
      {competitorSlots.map((slot, idx) => (
          <div key={`comp-${idx}`} className="concorrencia-catalog__cell concorrencia-catalog__cell--competitor">
            <CompetitorCompactCell
              ...
              contextualMuted={slot.contextualMuted}
...
export default function ConcorrenciaPage() {
  ...
  const {
    challengeCode: removeMonitoredCode,
    inputCode: removeMonitoredCodeInput,
    setInputCode: setRemoveMonitoredCodeInput,
    isConfirmed: canConfirmRemoveMonitoredCode,
    validationError: removeMonitoredValidationError,
    startChallenge: startRemoveMonitoredChallenge,
    resetChallenge: resetRemoveMonitoredChallenge,
  } = useDestructiveConfirmationCode();
  ...
  const canFetchCompetitionList = can(BILLING_ENTITLEMENT_CAPABILITY.VIEW_STORED_LISTS);

  useBillingEntitlementProfileTransition({
    onEnterExecutiveOnly: () => {
      setMonitoredListings([]);
      setCompetitionByMonitored({});
  ...
  }, [canFetchCompetitionList]);

  useEffect(() => {
    if (entitlementLoading) return undefined;
    let cancelled = false;
    (async () => {
      setListLoading(true);
  ...
  }, [loadCompetition, entitlementLoading]);
  ...
  const normalizedListFilterId = useMemo(
    () => normalizarIdFiltroRapidoConcorrencia(filterId),
    [filterId],
  );

  const filteredRows = useMemo(() => {
    const base =
      !normalizedListFilterId || normalizedListFilterId === "all"
        ? contextFiltered
        : contextFiltered.filter((row) => {
            ...
            return linhaAtendeFiltroListaConcorrencia(
              { competitors, competitorsCount: count, ownListing },
              normalizedListFilterId,
            );
          });

    if (isContextualCompetitorFilter(normalizedListFilterId)) {
      return ordenarLinhasPorMatchesContextuais(
        base,
        normalizedListFilterId,
        getCompetitors,
        getOwnListing,
        (row) => getConcorrenciaMonitoredRowSalesCount(row, getOwnListing),
      );
    }
  ...
  const handleListFilterChange = useCallback((nextFilterId) => {
    setFilterId(nextFilterId);
  }, []);

  const handleListSortChange = useCallback((nextSortId) => {
    setListSortId(nextSortId);
    setFilterId("all");
  }, []);
  ...
  const handleRequestRemoveMonitored = useCallback(
    (product) => {
      ...
      startRemoveMonitoredChallenge();
      setRemoveMonitoredTarget(product);
    },
    [removingMonitoredId, startRemoveMonitoredChallenge],
  );

  const handleCancelRemoveMonitored = useCallback(() => {
    if (removingMonitoredId) return;
    setRemoveMonitoredTarget(null);
    resetRemoveMonitoredChallenge();
  }, [removingMonitoredId, resetRemoveMonitoredChallenge]);
          ...
          <div className="products-catalog__list-header-slot">
            <div className="products-catalog__table-hscroll products-catalog__table-hscroll--head">
              <div className="concorrencia-catalog__grid concorrencia-catalog__grid--head">
                ...
                <div className="concorrencia-catalog__col-head concorrencia-catalog__col-head--account">Loja</div>
                ...
                <div className="concorrencia-catalog__body s7-operational-row-card-stack">
                  {paginatedRows.map((row) => (
                    <ConcorrenciaCatalogRow
                      ...
                      contextualFilterId={normalizedListFilterId}
          ...
          >
            ...
            <div className="concorrencia-catalog__confirm-code-block">
              <S7DestructiveConfirmationChallenge
                instruction="Digite o código de confirmação para habilitar a exclusão:"
                code={removeMonitoredCode}
                inputValue={removeMonitoredCodeInput}
                onInputChange={setRemoveMonitoredCodeInput}
                error={removeMonitoredValidationError}
              />
            </div>

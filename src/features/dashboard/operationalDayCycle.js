...
/**
 * Datas civis (YYYY-MM-DD) do ciclo no timezone do seller — só para UX do seletor.
 * Não substituem start_datetime/end_datetime da API.
 * @param {{
 *   startAt?: Date;
 *   endAt?: Date;
 *   timezone?: string;
 * } | null | undefined} cycle
 * @returns {{ startDate: string; endDate: string } | null}
 */
export function formatOperationalCycleCivilDates(cycle) {
  if (!cycle?.startAt || !cycle?.endAt) return null;
  const timezone =
    cycle.timezone != null && String(cycle.timezone).trim() !== ""
      ? String(cycle.timezone).trim()
      : DEFAULT_SELLER_TIMEZONE;
  const start = getZonedParts(cycle.startAt, timezone);
  const end = getZonedParts(cycle.endAt, timezone);

...
import { PRODUCT_EXPEDITION_SUPPLIES_LABEL, PRODUCT_EXPEDITION_SUPPLIES_TOOLTIP } from "../../../domain/costs/costSemanticsPresentation.js";
...
export default function ListingRayXModal({ open, listing, onClose }) {
  ...
  const detailFallbackFromRow = useMemo(
    ...
    [
      listing?.id,
      listing?.listingQualityScorePercent,
      listing?.listingQualityScore,
      listing?.initialQualitySnapshot,
      listing?.status,
      listing?.statusKey,
      listing?.visitCount,
      listing?.visits,
  ...
  const qualitySummary = useMemo(
    () =>
      normalizeListingQualitySummary(
        listing,
        editorDetail != null ? resumoDetail?.quality : null,
        listing?.initialQualitySnapshot ?? null,
        { detailLoaded: editorDetail != null },
      ),
    [listing, editorDetail, resumoDetail?.quality],
  );
  ...
  const custosTabFields = useMemo(
    () => [
      ...
      },
      {
        ...
        label: PRODUCT_EXPEDITION_SUPPLIES_LABEL,
        value: formatBrlFromApiString(costsSummary?.operational_cost_brl),
        tooltip: PRODUCT_EXPEDITION_SUPPLIES_TOOLTIP,
      },
    ],
    [costsSummary],
  );
          ...
          <div className="listing-rayx-modal__body">
            ...
            <div className="listing-rayx-modal__thumb-progress-row pf-right-progress-row pf-right-progress-row--with-thumb">
              ...
              <div className="listing-rayx-modal__thermometer pf-right-progress-semi">
                <ListingQualityGauge scorePercent={qualitySummary.score_percent} variant="sidebar" />
              </div>
            </div>

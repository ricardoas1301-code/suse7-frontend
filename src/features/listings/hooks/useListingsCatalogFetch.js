...
export function useListingsCatalogFetch({ onAfterLoad } = {}) {
  ...
  const canFetchLists = can(BILLING_ENTITLEMENT_CAPABILITY.VIEW_STORED_LISTS);
  ...
  const fetchListings = useCallback(async () => {
    ...
    const qualityByIdentity = buildListingQualityScoreByIdentityMap(mapped);
    const hydratedRows = joinCatalogRowsWithQualityIdentityMap(mapped, qualityByIdentity);
    setCatalogRows(hydratedRows);
    ...
    logListingsCatalogDev("payload_parsed", {
      elapsedMs,
      total: res.data?.total ?? hydratedRows.length,
    ...
    onAfterLoad?.();
    return true;
  }, [onAfterLoad, canFetchLists]);

  useEffect(() => {
    if (!authReady || entitlementLoading) {
      setListLoading(true);
  ...
  }, [authReady, entitlementLoading, canFetchLists, fetchListings]);

...
import { reconcileListingQualityScore } from "../domain/health/listingQualityHydration.js";
/**
 * @param {unknown} value
 */
 ...
 * @param {Record<string, unknown> | null | undefined} [openQualitySnapshot]
 * @param {{ detailLoaded?: boolean }} [options]
 */
export function normalizeListingQualitySummary(
  rawListing,
  listingHealthSource,
  openQualitySnapshot = null,
  options = {},
) {
  const source = listingHealthSource && typeof listingHealthSource === "object" ? listingHealthSource : {};
  const listingHydrated =
    rawListing != null && typeof rawListing === "object"
      ? /** @type {Record<string, unknown>} */ (rawListing)
      : null;

  const reconciled = reconcileListingQualityScore({
    detailLoaded: options.detailLoaded === true,
    detailScore: source.score_percent,
    snapshotScore:
      openQualitySnapshot?.score ?? openQualitySnapshot?.score_percent ?? listingHydrated?.initialQualitySnapshot?.score,
    listingScore: listingHydrated != null ? resolveCanonicalListingQualityScore(listingHydrated) : null,
  });

  const scorePercent = reconciled.score;
  ...
  return {
    ...
    source:
      options.detailLoaded === true && sourceRaw != null && sourceRaw !== "unavailable"
        ? sourceRaw
        : reconciled.source === "unavailable"
          ? "unavailable"
          : reconciled.source,
  };
}

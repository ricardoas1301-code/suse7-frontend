...
const FONTES_PROIBIDAS_CONFIRMACAO = new Set([
...
/**
 * Estado canônico do contrato de preço final — SSOT da aba Promoções PI.
 * @param {unknown} scenario
 * @returns {{
 *   status: "OFFICIAL_EXACT" | "PUBLIC_CONTRACT_UNAVAILABLE" | null;
 *   isUsable: boolean;
 ...
 * }}
 */
export function resolverEstadoVerdadePrecoPromocaoExibicao(scenario = null) {
  const { cardContract } = extrairCamadas(scenario);
  const truth =
    cardContract?.promotion_display_price_truth != null &&
    typeof cardContract.promotion_display_price_truth === "object"
      ? rec(cardContract.promotion_display_price_truth)
      : null;

  const statusRaw =
    truth?.official_promotion_display_price_truth_status ??
    cardContract?.official_promotion_display_price_truth_status ??
    null;
  const status =
    statusRaw === "OFFICIAL_EXACT" ||
    statusRaw === "PUBLIC_CONTRACT_UNAVAILABLE" ||
    statusRaw === "UNCONFIRMED"
      ? statusRaw
      : null;

  const isUsableExplicit = truth?.official_promotion_display_price_is_usable;
  const isConfirmedExplicit = truth?.official_promotion_display_price_is_confirmed;
  const isUsable =
    isUsableExplicit === true &&
    status === "OFFICIAL_EXACT" &&
    isConfirmedExplicit !== false;

  const isUnavailable =
    status === "PUBLIC_CONTRACT_UNAVAILABLE" ||
    status === "UNCONFIRMED" ||
    isUsableExplicit === false ||
    isConfirmedExplicit === false ||
    status !== "OFFICIAL_EXACT" ||
    cardContract?.official_promotion_display_price_brl == null;
  ...
  if (isUnavailable) {
    return {
      ...
      isUnavailable: true,
      reasonCode:
        truth?.official_promotion_display_price_reason_code != null
          ? String(truth.official_promotion_display_price_reason_code)
          : status === "UNCONFIRMED"
            ? "PUBLIC_CONTRACT_GAP"
            : "PUBLIC_CONTRACT_GAP",
      finalPriceBrl: null,
  ...
  const displayFinal = toDec(
    cardContract?.official_promotion_display_price_brl ?? truth?.official_promotion_display_price_brl,
  );
  ...
  if (displayFinal == null) {
    return {
      ...
      isUsable: false,
      isUnavailable: true,
      reasonCode: "PUBLIC_CONTRACT_GAP",
      finalPriceBrl: null,
    };
  }
...
export function resolvePromotionOfficialFinalPrice({ scenario = null } = {}) {
  ...
  if (contractState.isUnavailable) {
    warnings.push("public_contract_gap_display_price_unavailable");
    return {
      final_price_brl: null,
      selected_candidate_field: null,
      selected_candidate_reason: "public_contract_gap_no_fallback",
  ...
  const contaminado = cardContract?.contaminated_by_anonymous_price_discount === true;
  if (contaminado) {
    warnings.push("contaminated_by_anonymous_price_discount");
    return {
      final_price_brl: null,
      selected_candidate_field: null,
      selected_candidate_reason: "fail_closed_contaminated_anonymous_discount",
      used_generic_campaign_discount: false,
      warnings,
    };
  }

  // SSOT: o contrato canônico já resolveu identidade + semântica no backend.
  // Prioridade: official_promotion_display_price_brl (coluna Preço final da Central).
  const displayTruth =
    cardContract?.promotion_display_price_truth != null &&
    typeof cardContract.promotion_display_price_truth === "object"
      ? rec(cardContract.promotion_display_price_truth)
      : null;
  const displayFinal = toDec(
    cardContract?.official_promotion_display_price_brl ?? displayTruth?.official_promotion_display_price_brl
  );
  const displaySource =
    displayTruth?.source_field != null
      ? String(displayTruth.source_field)
      : cardSource;
  if (displayFinal != null) {
    if (displaySource != null && FONTES_PROIBIDAS_CONFIRMACAO.has(displaySource)) {
      warnings.push(`forbidden_display_source:${displaySource}`);
      return {
        final_price_brl: null,
        selected_candidate_field: null,
        selected_candidate_reason: "fail_closed_forbidden_source",
        used_generic_campaign_discount: false,
        warnings,
      };
    }
    return {
      final_price_brl: s2(displayFinal),
      selected_candidate_field: `promotion_card_contract.official_promotion_display_price_brl (${displaySource ?? "truth"})`,
      selected_candidate_reason: "canonical_official_promotion_display_price_ssot",
      used_generic_campaign_discount: false,
      warnings: [
        ...warnings,
        ...(Array.isArray(displayTruth?.warnings) ? displayTruth.warnings.map(String) : []),
      ],
    };
  }

  // S4.3.6.16 — FAIL-CLOSED: sem preço canônico oficial, NÃO usar
  // raw/sibling/simulado/desconto genérico. Retorna vazio.
  warnings.push("fail_closed_no_canonical_official_price");
  return {
    final_price_brl: null,
    selected_candidate_field: null,
    selected_candidate_reason: "fail_closed_unconfirmed_empty",
    used_generic_campaign_discount: false,
    warnings,
  };
}
  ...
  const warning =
    audit.resolver_used_generic_campaign_discount === true
      ...
      : audit.ui_vs_resolver_divergent === true
        ...
        : expected != null && resolver !== expected
          ? "resolver_ssot_differs_from_ml_expected_price"
          : null;
 ...
 */
function candidatePayloadIncludesExpectedPrice(audit, expected) {
  if (expected == null) return false;
  const rawCandidates = Array.isArray(audit.listing_specific_price_candidates)
    ? audit.listing_specific_price_candidates
    : [];
  for (const candidate of rawCandidates) {
    if (s2Value(candidate.value_brl) === expected) return true;
  }
  const fields = audit.candidate_fields != null && typeof audit.candidate_fields === "object"
    ? /** @type {Record<string, unknown>} */ (audit.candidate_fields)
    : {};
  for (const [key, value] of Object.entries(fields)) {
    if (!key.startsWith("raw_")) continue;
    if (s2Value(value) === expected) return true;
  }
  return false;
}
...
function diagnoseFreshness(audit) {
  ...
  if (expected != null && resolver !== expected) {
    const payloadHasExpected = candidatePayloadIncludesExpectedPrice(audit, expected);
    if (!payloadHasExpected) {
      if (audit.is_stale_vs_live === true) return "sync_cache_stale";
      return "server_payload_missing_official_price";
    }
    if (audit.ui_vs_resolver_divergent === true) return "frontend_candidate_priority";
    if (audit.is_stale_vs_live === true) return "sync_cache_stale";
    return "sync_cache_stale";
  }

 ...
 *   precoPromocionalAtualBrl?: string | null;
...
export function reidratarReducaoTarifaNoCenarioFinal(cenarioSim, promocaoFonte, ctx = {}) {
  ...
  const precoAtualDec =
    ctx.precoPromocionalAtualBrl != null && String(ctx.precoPromocionalAtualBrl).trim() !== ""
      ? toDec(String(ctx.precoPromocionalAtualBrl).replace(/[^\d,.-]/g, "").replace(",", "."))
      : toDec(cenarioSim.sale_price_brl ?? mAntesCtx.sale_price_brl);

  const gateOficial = buildFinalPromotionTruthPresentation({ scenario: promocaoFonte });
  const precoOficialDec =
    gateOficial.truthStatus === "CONFIRMED_OFFICIAL"
      ? toDec(gateOficial.officialPriceBrl)
      : null;

  const simPayoutAntesDec =
    toDec(mAntesCtx.marketplace_payout_amount_brl) ??
    toDec(mAntesCtx.net_receivable_brl) ??
    toDec(cenarioSim.net_receivable_brl);

  if (
    precoAtualDec != null &&
    precoOficialDec != null &&
    precoAtualDec.gt(0) &&
    precoOficialDec.gt(0) &&
    precoAtualDec.minus(precoOficialDec).abs().gt(TOLERANCIA_BRL)
  ) {
    return cenarioSim;
  }
  ...
  if (
    ...
    precoAtualDec != null &&
    precoAtualDec.gt(0) &&
    simPayoutAntesDec != null &&
    simPayoutAntesDec.gt(0)
  ) {
    return cenarioSim;
  }

...
import {
  ...
  formatOperationalCycleCivilDates,
...
export function resolveDailySummaryScope(filters, operationalConfig = {}, scopeOptions = {}) {
  ...
  const operationalCivilDates = resumoUsesOperationalCycle
    ? formatOperationalCycleCivilDates(operationalCycle)
    : null;
  ...
  return {
    ...
    resumoFilterParams,
    operationalCycle,
    operationalCivilDates,
    operationalConfig: { closesAt, timezone, workingDays },
    appliedRange,
    /** Datas exibidas no seletor: civis do ciclo quando operacional; senão o filtro manual. */
    selectorStartDate: operationalCivilDates?.startDate ?? appliedRange.startDate,
    selectorEndDate: operationalCivilDates?.endDate ?? appliedRange.endDate,
    selectorTriggerLabel: resumoUsesOperationalCycle ? "Período: ciclo operacional" : null,
  };
}

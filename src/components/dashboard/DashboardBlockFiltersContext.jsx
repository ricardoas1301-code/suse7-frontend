 ...
 *   applyDailySummaryPeriod: (payload: {
 *     preset: VendasPeriodPresetUi | "operational_cycle";
 *     startDate: string;
 *     endDate: string;
 *   }) => void;
 *   resetDailySummaryToOperationalCycle: () => void;
...
export function DashboardBlockFiltersProvider({ children }) {
  ...
  const applyDailySummaryPeriod = useCallback(
    /** @param {{ preset: VendasPeriodPresetUi | "operational_cycle"; startDate: string; endDate: string }} payload */ (
      payload,
    ) => {
      if (payload.preset === "operational_cycle") {
        // Volta ao ciclo operacional canônico (fonte = operationalConfig).
        setDailySummaryPeriodTouched(false);
        return;
      }
  ...
  const resetDailySummaryToOperationalCycle = useCallback(() => {
    setDailySummaryPeriodTouched(false);
  }, []);
  ...
  const value = useMemo(
    () => ({
      ...
      resetDailySummaryToOperationalCycle,

...
import { formatarPercentualDescontoExatoExibicao } from "../../features/pricing/promotions/promotionDiscountSemantics.js";

const VIGENCIA_NAO_INFORMADA = "Vigência não informada";
...
function normalizarTextoPeriodoUpper(line) {
  const t = line.trim();
  if (t === "") return VIGENCIA_NAO_INFORMADA;
  const lc = t.toLowerCase();
  if (lc === "sem data informada" || lc === "vigência não informada" || lc === "vigencia nao informada") {
    return VIGENCIA_NAO_INFORMADA;
  }
...
export function resolverPeriodoMiniCardPromocao(scenario, isRelampago = false) {
  if (scenario == null || typeof scenario !== "object") {
    return isRelampago ? POR_TEMPO_LIMITADO : VIGENCIA_NAO_INFORMADA;
  }
  const r = /** @type {Record<string, unknown>} */ (scenario);
...
function resolverDescontoDerivadoDoPrecoFinal(scenario) {
  ...
  try {
    ...
    // S4.3.6.18 — % exato sem colapsar para 2 casas (evita confundir com o inteiro visual do ML).
    const percent = amount.div(original).times(100);
...
/**
 * Percentual visual do marketplace (payload) — observação apenas.
 * Não usar para reconstruir preço oficial / simulação.
 * @param {unknown} scenario
 * @returns {Decimal | null}
 */
export function resolverPercentualDescontoVisualMarketplace(scenario) {
  if (scenario == null || typeof scenario !== "object") return null;
  const contract = obterContratoPromocaoCanonica(scenario);
  const brutoExibicao = contract?.discount_percent_display;
  if (brutoExibicao == null || String(brutoExibicao).trim() === "") return null;
...
/**
 * S4.3.6.18 — Percentual matemático exato derivado do preço oficial (SSOT).
 * Não usa discount_percent_display (inteiro visual do ML).
 * @param {unknown} scenario
 * @returns {Decimal | null}
 */
...
export function resolverRotuloDescontoMiniCardPromocao(scenario) {
  ...
  // S4.3.6.18 — rótulo com % exato (precisão contratada), não o inteiro visual do ML.
  const exato = formatarPercentualDescontoExatoExibicao(pct);
  return `Desconto de ${exato ?? formatarPercentualPromocaoPtBr(pct)}%`;
}
